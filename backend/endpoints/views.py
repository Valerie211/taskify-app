...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def shared_with_task_counts(request):
    try:
        ...
        # Total tasks completed by the shared_with user
        total_tasks_completed = Task.objects.filter(progress_status='complete', shared_with__id=shared_with_user_id).count()
        
        # Total tasks pending by the shared_with user
        total_tasks_pending = Task.objects.filter(progress_status='pending', shared_with__id=shared_with_user_id).count()
        
        # Total tasks not assigned by the shared_with user
        total_tasks_not_assigned = Task.objects.filter(progress_status='not_assigned', shared_with__id=shared_with_user_id).count()
        
        # Total tasks in progress by the shared_with user
        total_tasks_in_progress = Task.objects.filter(progress_status='progress', shared_with__id=shared_with_user_id).count()
        
        # Count of tasks assigned on a month-by-month basis
        monthly_assigned_tasks = Task.objects.filter(
        ...
        return Response({
            ...
            "total_tasks_not_assigned": total_tasks_not_assigned,
            "total_tasks_in_progress": total_tasks_in_progress,
            "monthly_assigned_tasks": monthly_assigned_tasks
        }, status=status.HTTP_200_OK)
...
def edit_task(request, task_id):
    ...
    try:
        ...
        if request.method == 'PUT':
            ...
            task.shared_with.set(request.data.get('shared_with_users', task.shared_with.all()))
            task.save()
