...
import {getSharedWithCountApi, getTaskApiById, updateTaskApi} from "../../utils/tasks"
...
const TeamMemberTask = () => {
    ...
    const getTasks =async()=>{
        ...
        try{
            ...
            const sharedWithCounts = await getSharedWithCountApi()
            console.log("shared",sharedWithCounts)

            console.log("tasks-results",result.data)
            if(result.status === 200){
                setData(result?.data)
                const taskAssignedToTeamMember= result?.data.task_list.filter((user_project,i)=>
                user_project.shared_with.some(item =>item.id ===user))
                setTaskList(taskAssignedToTeamMember)
                setCounts(sharedWithCounts.data)
                setLoading(false)
            }
        ...
        <div className="px-4 relative">
            ...
            <div className="flex  px-4 w-full h-20 bg-white mt-4">
                <div className="flex w-1/3 items-center justify-center py-2 border-r">
                    <Cards amount={counts?.total_tasks_not_assigned}
                        color={"blue"}
                        title={"Not Started"}
                ...
                <div className="flex w-1/3 items-center justify-center py-2 border-r">
                    <Cards amount={counts?.total_tasks_in_progress}
                        color={"purple"}
                        title={"In progress"}
                        text={"tasked assigned"}
                    />  
                ...
                <div className="flex w-1/3 items-center justify-center py-2 border-r">
                    <Cards amount={counts?.total_tasks_completed}
                    color={"green"}
                        title={"Complete"}
                        text={"tasked assigned"}
                    />
